...
def OP(I, R, Raza, k, Pk, priceSmall, priceBig):
    ...
    Pi = {i: rnd.randint(1, 8) for i in I}
    Pi.update({i: 0 for i in R1})
    # print(Pi) #Pick-up amount
    Di = {i: rnd.randint(1, 5) for i in I}
...
def Tabel(grid, DateTabel, PretCentru, TipDroneCentru, DistantaTotala, NrDroneMici, NrDroneMari, PretTotal, IdPuncte, nrPacienti, timp):
    grid.CreateGrid(len(PretCentru)+2, 7)
    ...
    grid.SetCellValue(1, 0, 'Centru de Drone')
    grid.SetCellValue(1, 1, str(nrPacienti)+' Pacienti')
    grid.SetCellValue(1, 2, ' %d (Tip I: %d Tip II: %d)' %
                      (NrDroneMici+NrDroneMari, NrDroneMici, NrDroneMari))
    grid.SetCellValue(1, 3, str(PretTotal)+'$')
    grid.SetCellValue(1, 5, str(round(DistantaTotala, 2)))
    grid.SetCellValue(1, 6, str(round(timp, 2))+' ms')
    PacientiCentru = dict()
    for i in DateTabel.keys():
